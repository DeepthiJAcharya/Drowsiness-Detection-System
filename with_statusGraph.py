...
def get_db():
    ...
    if db is None:
        db = g._database = sqlite3.connect('driver_status.db', check_same_thread=False) #one one val
    return db
...
def generate_frame():
    ...
    imageBackground = cv2.imread('img/imgbg1.png')
    resized_image = cv2.resize(imageBackground, (width, height))

    # importing the mode images into a list
    folderModePath = 'img/modes'
    ...
    s = cv2.imread('img/modes/mode 2.png')
